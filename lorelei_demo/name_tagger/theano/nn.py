...
class MultiLeNetConvLayer(object):

    def __init__(self, input, filter_shapes, pool_sizes, name='conv'):

        rng = np.random.RandomState(2345)
        self.name = name
        self.input = input
        self.convW1 = theano.shared(
            np.asarray(rng.uniform(low=-0.25, high=0.25, size=filter_shapes[0]),
                       dtype=theano.config.floatX), name='convW1', borrow=True
        )
        self.convW2 = theano.shared(
            np.asarray(rng.uniform(low=-0.25, high=0.25, size=filter_shapes[1]),
                       dtype=theano.config.floatX), name='convW2', borrow=True
        )
        self.convW3 = theano.shared(
            np.asarray(rng.uniform(low=-0.25, high=0.25, size=filter_shapes[2]),
                       dtype=theano.config.floatX), name='convW3', borrow=True
        )
        self.convB1 = theano.shared(
            value=np.zeros((filter_shapes[0][0],), dtype=theano.config.floatX),
            name='convB1', borrow=True
        )
        self.convB2 = theano.shared(
            value=np.zeros((filter_shapes[1][0],), dtype=theano.config.floatX),
            name='convB2', borrow=True
        )
        self.convB3 = theano.shared(
            value=np.zeros((filter_shapes[2][0],), dtype=theano.config.floatX),
            name='convB3', borrow=True
        )

        conv_output1 = self.encoder_cnn(input, filter_shapes[0], pool_sizes[0],
                                        self.convW1, self.convB1)
        conv_output2 = self.encoder_cnn(input, filter_shapes[1], pool_sizes[1],
                                        self.convW2, self.convB2)
        conv_output3 = self.encoder_cnn(input, filter_shapes[2], pool_sizes[2],
                                        self.convW3, self.convB3)

        self.output = T.concatenate((conv_output1, conv_output2, conv_output3),
                                    axis=1)

        self.params = [self.convW1, self.convB1, self.convW2, self.convB2,
                       self.convW3, self.convB3]

    def encoder_cnn(self, layer0_input, filter_shape,
                    pool_size, convW, convB): ##='cnn_encoder'
        ...
        """
        conv_out = conv.conv2d(input=layer0_input, filters=convW,
                               filter_shape=filter_shape)
        conv_out_tanh = T.tanh(conv_out + convB.dimshuffle('x', 0, 'x', 'x'))
        output = downsample.pool_2d(input=conv_out_tanh, ws=pool_size,
                                    ignore_border=True)
